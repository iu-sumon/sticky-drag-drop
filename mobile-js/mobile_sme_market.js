...
function mobile_sme_market() {
    var sme_list = document.getElementById("sme_list");
    sme_list.innerHTML = '';

    $.getJSON("/shared/mobile_sme_market", function (data) {
        var color_class = '';
        var pos_neg = '';

        for (i = 0; i < data.sme_symbol_list.length; i++) {

            symbol = data.sme_symbol_list[i].symbol;
            board = data.sme_symbol_list[i].board;
            symname = symbol + board;
            sym_board = symbol + "." + board;

            category = data.sme_symbol_list[i].symbol_category;
            sector = data.sme_symbol_list[i].sector;
            market_type = data.sme_symbol_list[i].market_type;
            symbol_type = data.sme_symbol_list[i].symbol_instr;

            ltp = data.sme_ltp_list[i].ltp == null ? '-' : parseFloat(data.sme_ltp_list[i].ltp);
            ycp = data.sme_ltp_list[i].ycp == null ? '-' : parseFloat(data.sme_ltp_list[i].ycp);
            change = data.sme_ltp_list[i].ltp_change == null ? '-' : parseFloat(data.sme_ltp_list[i].ltp_change);
            changeper = data.sme_ltp_list[i].ltp_changeper == null ? '-' : parseFloat(data.sme_ltp_list[i].ltp_changeper);


            if (data.sme_eod_list.length > 0) {
                eod_obj = data.sme_eod_list.find(idx => (idx.symbol == symbol && idx.board == board));
            ...
            vol = data.sme_ltp_list[i].total_qty == null ? '-' : Number(data.sme_ltp_list[i].total_qty).toLocaleString("en-IN");
            value = data.sme_ltp_list[i].total_value == null ? '-' : parseFloat(data.sme_ltp_list[i].total_value);

            var value_tag = '';
            if (data.sme_ltp_list[i].total_value != null) {
                if (value > 10000000) {
            ...
            if (sme_list != undefined) {

                var item = document.createElement("li");
                item.classList.add('watchlist-mobile-table-row-style');
                item.innerHTML = `
                <div onclick="onCompanySelect('${symbol}','${sym_board}','mobile_sme_page')" class="table-first-column watchlist-row-content-align row-content-start">
                      <div>
                         <div>
                ...
                document.getElementById("dom-watchlist").appendChild(sme_list);
            }
        }
        clones = $("#sme_list li").clone(true);
    });
}
...
$('.back-button-sme-mkt').on('click', function () {
