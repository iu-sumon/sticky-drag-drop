...
function mobile_atb_market() {
    var atb_list = document.getElementById("atb_list");
    atb_list.innerHTML = '';

    $.getJSON("/shared/mobile_atb_market", function (data) {
        var color_class = '';
        var pos_neg = '';

        for (i = 0; i < data.atb_symbol_list.length; i++) {

            symbol = data.atb_symbol_list[i].symbol;
            board = data.atb_symbol_list[i].board;
            symname = symbol + board;
            sym_board = symbol + "." + board;

            category = data.atb_symbol_list[i].symbol_category;
            sector = data.atb_symbol_list[i].sector;
            market_type = data.atb_symbol_list[i].market_type;
            symbol_type = data.atb_symbol_list[i].symbol_instr;

            ltp = data.atb_ltp_list[i].ltp == null ? '-' : parseFloat(data.atb_ltp_list[i].ltp);
            ycp = data.atb_ltp_list[i].ycp == null ? '-' : parseFloat(data.atb_ltp_list[i].ycp);
            change = data.atb_ltp_list[i].ltp_change == null ? '-' : parseFloat(data.atb_ltp_list[i].ltp_change);
            changeper = data.atb_ltp_list[i].ltp_changeper == null ? '-' : parseFloat(data.atb_ltp_list[i].ltp_changeper);


            if (data.atb_eod_list.length > 0) {
                eod_obj = data.atb_eod_list.find(idx => (idx.symbol == symbol && idx.board == board));
            ...
            vol = data.atb_ltp_list[i].total_qty == null ? '-' : Number(data.atb_ltp_list[i].total_qty).toLocaleString("en-IN");
            value = data.atb_ltp_list[i].total_value == null ? '-' : parseFloat(data.atb_ltp_list[i].total_value);

            var value_tag = '';
            if (data.atb_ltp_list[i].total_value != null) {
                if (value > 10000000) {
            ...
            if (atb_list != undefined) {

                var item = document.createElement("li");
                item.classList.add('watchlist-mobile-table-row-style');
                item.innerHTML = `
                <div onclick="onCompanySelect('${symbol}','${sym_board}','mobile_atb_page')" class="table-first-column watchlist-row-content-align row-content-start">
                      <div>
                         <div>
                ...
                document.getElementById("dom-watchlist").appendChild(atb_list);
            }
        }
        clones = $("#atb_list li").clone(true);
    });
}
...
$('.back-button-atb-mkt').on('click', function () {
